...
void * http_accept_loop(void *arg){
  ...
  server->beginAcceptLoop(event_thread_id);
}
...
int main(int argc, char *argv[]){
  int ret;
  int i=0;
  pthread_t event_threads[THREAD_NUM];
  ...
  server = new Server(80, THREAD_NUM);

  ret = server->Init();
  if(ret == -1){ printf("Error: Server->Init() returned -1\n"); fflush(stdout); exit(-1); }

  for(i=0; i<THREAD_NUM; i++){
    event_thread_ids[i] = i;
  }

  for(i=0; i<THREAD_NUM; i++){
    ret = pthread_create(&accept_threads[i], NULL, &http_accept_loop, &event_thread_ids[i]);
    if(ret != 0){
